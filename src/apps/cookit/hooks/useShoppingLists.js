...
const STORAGE_KEY = 'cookit.activeListId'
// L'app s'appelait « courses » : on lit encore l'ancienne cle une fois, sinon
// le renommage ferait perdre la liste selectionnee au prochain chargement.
const LEGACY_STORAGE_KEY = 'courses.activeListId'
...
export function useShoppingLists() {
  ...
  useEffect(() => {
    ...
    if (lists.length === 0) {
      ...
      assignLegacyItems(id, currentUid).catch((err) => console.error('[Cook’It] migrate items:', err))
      setSelectedId(id)
      writeStored(id)
    }
  ...
  function handleRename(id, name) {
    renameList(id, name, currentUid).catch((err) => console.error('[Cook’It] renameList:', err))
  }
  function handleArchive(id) {
    setListStatus(id, 'archived', currentUid).catch((err) => console.error('[Cook’It] archiveList:', err))
  }
  function handleUnarchive(id) {
    setListStatus(id, 'active', currentUid).catch((err) => console.error('[Cook’It] unarchiveList:', err))
    selectList(id)
  }
  function handleDelete(id) {
    deleteList(id).catch((err) => console.error('[Cook’It] deleteList:', err))
  }
