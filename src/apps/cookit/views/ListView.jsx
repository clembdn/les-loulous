...
export default function ListView({ items, catalog, pantry, foods = [], isLoading, listsApi, counts, onManageLists, onLogFood }) {
  ...
  const handleAdd = (name) => addNamedItem({ name }, { catalog, currentUid, items, listId: activeListId, foods })
  function handleToggle(it) {
    ...
    if (willCheck) {
      ...
      // Si l'article est lié à un aliment connu, on propose de le journaliser —
      // sans l'imposer : cocher veut dire « acheté », pas « mangé ».
      const food = it.foodId ? foods.find((f) => f.id === it.foodId) : null
      if (food) {
        toast(food.name, {
          description: 'Ajouter au journal ?',
          action: { label: 'Journaliser', onClick: () => onLogFood?.(food) },
        })
      }
    }
  }
  ...
  return (
    <div className="max-w-xl lg:max-w-5xl mx-auto px-4 pb-44 lg:pb-28 pt-4">
      ...
      <div className="lg:max-w-2xl">
        <QuickAddBar catalog={catalog} items={items} onAdd={handleAdd} onPickFood={() => setFoodPickerOpen(true)} />
      </div>
      ...
      <FoodPickerSheet
        open={foodPickerOpen}
        foods={foods}
        title="Ajouter un aliment à la liste"
        onClose={() => setFoodPickerOpen(false)}
        onPick={(food) => {
          addNamedItem(
            { name: food.name, foodId: food.id },
            { catalog, currentUid, items, listId: activeListId, foods },
          )
