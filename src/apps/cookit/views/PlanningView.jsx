...
export default function PlanningView({ recipes, items, catalog, pantry, foods = [], foodById, activeListId, onGoToList }) {
      ...
      for (const slot of ['midi', 'soir']) {
        for (const meal of d[slot] || []) {
          ...
          for (const ing of r.ingredients) {
            ...
            if (!map.has(key)) {
              map.set(key, { name: ing.name, quantity: incoming.quantity, unit: incoming.unit, foodId: ing.foodId || null })
            } else {
              const cur = map.get(key)
              const merged = mergeQuantity(cur, incoming)
              // Le premier lien rencontre gagne : deux recettes qui citent le meme
              // ingredient le pointent de toute facon vers le meme aliment.
              map.set(key, { name: cur.name, quantity: merged.quantity, unit: merged.unit, foodId: cur.foodId || ing.foodId || null })
            }
          }
        }
      }
  ...
  return (
    <div className="max-w-xl lg:max-w-6xl mx-auto px-4 pb-44 lg:pb-28 pt-4">
      <h1 className="text-xl font-semibold tracking-tight text-fg mb-3">Planning</h1>
      <div className="flex items-center justify-between mb-5 rounded-xl border border-border bg-surface px-2 py-1.5 lg:max-w-md lg:mx-auto">
      ...
      <div>
        {isLoading ? (
          ...
          <div className="space-y-5 lg:space-y-0 lg:grid lg:grid-cols-7 lg:gap-3">
            {week.days.map((day) => {
              const d = dayMap[day.id]
              return (
                <section key={day.id}>
