...
import { addNamedItems } from '../utils/addItems.js'
...
export default function AddIngredientsSheet({
  open, onClose, ingredients, items, catalog, pantry = [], foods = [], listId = null, onAdded, title = 'Ajouter à la liste',
}) {
  ...
  function confirm() {
    const chosen = list.filter((_, i) => checked[i]).map((ing) => ({
      name: ing.name,
      quantity: ing.quantity,
      unit: ing.unit,
      quantityLabel: ing.quantityLabel,
      // Le lien nutritionnel voyage jusqu'a la liste de courses.
      foodId: ing.foodId || null,
    }))
    addNamedItems(chosen, { catalog, currentUid, items, listId, foods })
    onClose()
    onAdded?.()
  }
