...
import { ArrowLeft, Plus, X, ImagePlus, Apple } from 'lucide-react'
...
export default function RecipeEditor({ recipe, foods = [], foodById, onCancel, onSave }) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-bg">
      ...
      <div className="max-w-xl mx-auto px-4 pb-16 pt-4 space-y-6">
        ...
        <div>
              ...
              <div key={i} className="flex flex-wrap gap-2 items-center">
                ...
                <div className="flex gap-2 items-center">
                  ...
                  <button
                    ...
                    aria-label={ing.foodId ? 'Remplacer l’aliment' : 'Choisir un aliment'}
                    title={ing.foodId ? foodById?.get(ing.foodId)?.name || 'Aliment lié' : 'Choisir un aliment scanné'}
                    className={cn(
                      'shrink-0 p-2 transition',
                      ing.foodId ? 'text-accent' : 'text-faint hover:text-accent',
                    )}
                  >
                    <Apple size={16} />
                  </button>
                ...
                {ing.foodId && (
                  <span className="w-full -mt-1 pl-1 text-[11px] text-accent truncate">
                    {foodById?.get(ing.foodId)?.name || 'Aliment lié'}
                  </span>
                )}
              </div>
            ))}
          ...
          <div className="mt-3 flex flex-wrap items-center gap-4">
            ...
            <button onClick={() => setPickingFor('new')} className="inline-flex items-center gap-1.5 text-sm text-muted hover:text-accent transition">
              <Apple size={16} /> Depuis ma bibliothèque
            </button>
          </div>
        </div>
      ...
      <FoodPickerSheet
        ...
        foods={foods}
        title={pickingFor === 'new' ? 'Ajouter un ingrédient' : 'Aliment de cet ingrédient'}
