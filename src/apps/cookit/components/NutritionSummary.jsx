...
export default function NutritionSummary({ totals, unresolved = [], label, onFix, className }) {
  ...
  return (
    <div className={cn('rounded-2xl border border-border bg-surface p-4', className)}>
      <div className="flex items-baseline justify-between gap-3 mb-3">
        <span className="text-2xl font-semibold text-fg tabular">{formatKcal(totals?.kcal)}</span>
      ...
      <div className="grid grid-cols-3 gap-2">
        {MACROS.map((m) => (
          <div key={m.key} className="min-w-0">
            ...
            <span className="block text-sm text-fg tabular">{formatGrams(totals?.[m.key])}</span>
          </div>
        ))}
      </div>
