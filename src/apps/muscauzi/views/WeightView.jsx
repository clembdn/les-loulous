...
import { toDateId, fromDateId, formatDayFr, formatDateShortFr, formatDateFr } from '@/shared/lib/dates.js'
  ...
  return (
    <div className="max-w-xl mx-auto px-4 pt-5 pb-28 lg:pb-10 lg:pt-8 lg:px-6">
      <header className="mb-5">
        <p className="text-xs uppercase tracking-[0.18em] text-faint">Poids corporel</p>
        <h1 className="text-2xl font-semibold tracking-[-0.02em] text-fg mt-1">Ma courbe</h1>
      </header>
...
function formatDelta(value, last) {
  ...
  const sign = delta > 0 ? '+' : ''
  const when = formatDateFr(fromDateId(last.date))
