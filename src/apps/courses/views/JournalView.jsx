...
import { addEntry, removeEntry, SLOTS } from '../services/foodLogService.js'
...
export default function JournalView({ foods, recipes, foodById, goals }) {
  const { currentUid } = useAuth()
  const today = toLocalDateKey()
  const [dateId, setDateId] = useState(today)
  const { entries, isLoading } = useFoodLog(dateId)
  ...
  const totals = useMemo(() => entries.reduce((acc, e) => ({
    ...
    fat: acc.fat + e.fat,
  }), { kcal: 0, proteins: 0, carbs: 0, fat: 0 }), [entries])

  const bySlot = useMemo(() => {
    const m = Object.fromEntries(SLOTS.map((s) => [s.id, []]))
    entries.forEach((e) => { (m[e.slot] ||= []).push(e) })
    return m
  }, [entries])
  ...
  function addRecipePortions(recipe, portions) {
    ...
    <div className="max-w-xl mx-auto px-4 pb-28 pt-2">
          ...
          <section key={s.id}>
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-xs text-faint tabular">
                {formatKcal(bySlot[s.id].reduce((a, e) => a + e.kcal, 0))}
              </span>
            </div>
            <ul className="divide-y divide-border">
              {bySlot[s.id].map((e) => (
                <li key={e.id} className="flex items-center gap-3 py-2 group">
                  ...
                  <span className="text-sm text-muted tabular shrink-0">{Math.round(e.kcal)}</span>
                  <button
                    onClick={() => removeEntry(currentUid, dateId, e.id)}
            ...
            <button
              ...
              className="mt-1 inline-flex items-center gap-1.5 text-xs text-muted hover:text-accent transition"
            >
      ...
      <div className="fixed bottom-16 lg:bottom-0 inset-x-0 lg:left-60 z-20 p-4 pointer-events-none">
        <div className="max-w-xl mx-auto flex justify-end gap-2">
          <Button variant="secondary" className="pointer-events-auto shadow-lift" onClick={() => { setPickerSlot('midi'); setRecipeSheet(true) }}>
            <ChefHat size={16} /> Un plat
          </Button>
          <Button className="pointer-events-auto shadow-lift" onClick={() => setPickerSlot('midi')}>
