...
import { searchProducts, fetchByBarcode } from '../services/openFoodFactsService.js'
...
export default function FoodPickerSheet({ open, foods, onClose, onPick, title = 'Choisir un aliment' }) {
  ...
  useEffect(() => {
    if (!open) { setQ(''); setCiqual([]); setOff([]); setScanMiss(null) }
  }, [open])
  ...
  function pick(food) {
    ...
    if (known) recordFoodUsage(id, currentUid)
    onPick({ ...food, id })
  }
  ...
  async function onScanned(barcode) {
    ...
    // Produit absent d'OFF (fréquent en Australie) : on ouvre directement la
    // fiche vierge, code-barres pré-rempli, pour ne pas scanner dans le vide.
    setScanMiss(barcode)
    setDraft({ barcode, source: 'off', name: '', per100: {} })
  }

  return (
    <>
      <Sheet open={open && !draft} onOpenChange={(o) => !o && onClose()} title={title}>
        <div className="space-y-4">
          ...
          {scanMiss && (
            <p className="text-xs text-muted">
              Code-barres {scanMiss} inconnu d’Open Food Facts.
            </p>
          )}
